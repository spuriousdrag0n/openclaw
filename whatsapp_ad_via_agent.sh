#!/bin/bash
# Routes ad send request through agent session
...
# Send command to agent via Telegram
$OPENCLAW message send \
    --channel telegram \
    --target "5442103415" \
    --message "AGENT_TASK: Send WhatsApp ad to $TARGET

Use message tool:
--channel whatsapp
--target $TARGET
--media /root/.openclaw/media/inbound/file_0---f34d7c03-e4a1-46d0-8f11-724dce29292c.jpg
...
After sending, confirm success and I will update state."

if [ $? -eq 0 ]; then
    echo "{\"index\": $NEXT, \"last\": \"$TARGET\", \"time\": \"$(date -Iseconds)\"}" > "$STATE_FILE"
fi
